...
**Polish (mostly done):**
7. ~~Fly-in animation on slider increment.~~ ✅ drop-from-above + landing pulse in `Scene3D.tsx`
8. ~~Staggered UI reveal + beige/forest-green theme.~~ ✅ `globals.css` + `reveal-*` classes on sidebar/charts
9. ~~Adaptive Design tab on real fused matrix.~~ ✅
10. Demo rehearsal 3×; projector screenshot — **still TODO**
11. Wire `FilterControls.tsx` into main Explore sidebar (component exists; API `/custom-panel` ready)

### Sister (Natasha) — DONE

1. ~~Download metabolomics~~ ✅ from CellMiner → `r/loading.R` → `metabolomics_log_zscored.csv`
2. ~~Fix `r/loading.R`~~ ✅ `common_cell_lines`; paths under `processed_data/`
3. ~~Export `sample_info.csv`~~ ✅
4. ~~Confirm all layers aligned on same 60 lines after metabolomics join~~ ✅
5. ~~**fgsea** on RNA PCA loadings~~ ✅ → `factor_annotations.json` + `pathway_scores.json`
6. ~~**Characterization** for selected lines~~ ✅ → `characterization.json`
7. ~~Re-run blindspot with real pathway scores (Level 2 blind spots)~~ ✅

### Both (sync)

- [x] Pick fusion layers: **rna_seq, proteomics, metabolomics, drug_activity** (+ methylation/histone in current 5-layer R fused matrix)
- [x] Download + clean metabolomics (sister)
...
Animations:
- When panel size changes: new sphere **flies in from above** (2× offset + Y+14), easeOutBack landing, brief scale pulse — **✅ implemented**
- When cancer filter changes: non-matching spheres fade out smoothly (opacity 0.15) — **✅**
- On page load: sidebar, charts, and compare panels **stagger-reveal** upward (`reveal-up` keyframes, respects `prefers-reduced-motion`) — **✅**
- On hover: sphere scales up slightly, tooltip appears — **✅**
- Connection lines between selected spheres (thin gold lines) — **✅**
- Slow ambient rotation when user not interacting — **✅**
- Red rim pulse on blind-spot cancer types — **✅**
...
### Blind Spot Detector (Stretch — ✅ DONE on real data)
...
**Level 2 — Pathway coverage** (✅ real `pathway_scores.json` from sister fgsea):
...
| Log2 + z-score all layers | 2-3 | `processed_data/log_zscored/` | ✅ `check_log_zscore.R` (smart detect) |
| Z-score methylation/proteomics | 2-3 | `processed_data/zscored/` | ✅ |
| Filter to 150 landmark drugs | 3-4 | `processed_data/clean/drug_activity_landmark_*` | ✅ `filter_landmark_drugs.R` |
| PCA per layer + fuse | 4-6 | `processed_data/pca/fused_matrix.csv` | ✅ `pca_analysis.R` |
| Export `sample_info.csv` (cancer types) | 3-4 | `data/processed/sample_info.csv` | ✅ + `build_sample_info.py` |
| Top-feature annotation per PC | 8-12 | `processed_data/annotation/` | ✅ `biological_annotation.R` |
| Run fgsea on RNA PCA loadings | 8-12 | `factor_annotations.json` | ✅ |
| Characterization JSON | 14-18 | `characterization.json` | ✅ |
| Pathway scores per cell line | 14-18 | `pathway_scores.json` | ✅ |
| Download + clean metabolomics | 3-4 | `metabolomics_log_zscored.csv` | ✅ |
...
| Drug sensitivity validation correlation | 8-11 | validation.json | ✅ Ridge regression |
...
| Fly-in animation for new sphere selections | 10-12 | Scene3D.tsx | ✅ drop + easeOutBack + pulse |
...
| Dark theme polish, animations, responsive sidebar | 24-28 | global styles | ✅ beige/green theme + reveal stagger |
| Analyze Your Data tab (upload + protein search) | stretch | CustomDataAnalysis.tsx, custom_analysis.py | ✅ |
| User filters + layer weights | stretch | FilterControls.tsx, customization.py | ✅ API; UI not wired to Explore |
| Ridge regression validation | 8-11 | validation.py | ✅ |
...
frontend/public/precomputed/
  ...
  characterization.json     ← biological characterization per cell line       ✅
  factor_annotations.json   ← PCA factor labels (fgsea Hallmark)             ✅
  blindspot.json            ← cancer type + pathway blind spots              ✅
  embeddings.json           ← fused PCA coords for manual coverage           ✅
  pathway_scores.json       ← per-cell-line pathway scores (fgsea)           ✅
  adaptive_design.json      ← stepwise policy rollouts (real fused matrix) ✅
  protein_expression.json   ← per-protein levels across NCI-60 for search tab ✅
```
...
### You
- [x] Download NCI-60 datasets from CellMiner (in `raw_data/` — **including metabolomics**)
...
- [x] fgsea + characterization → `factor_annotations.json`, `pathway_scores.json`, `characterization.json`
...
| Three.js fly-in animation not smooth | ✅ Implemented with easeOutBack + pulse; simplify to instant highlight if demo machine struggles |
